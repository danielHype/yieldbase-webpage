...
const YieldGenerator = () => {
    ...
    const ensureSevenDays = (dataPoints) => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const existingData = dataPoints.find(
                (point) => format(point.x, 'yyyy-MM-dd') === format(currentDay, 'yyyy-MM-dd')
            );
            if (existingData) {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            setChartData((prevData) => {
                ...
                if (!lastDataPoint || format(lastDataPoint.x, 'yyyy-MM-dd') !== format(todayStart, 'yyyy-MM-dd')) {
                    newData.push({
                        x: todayStart,
                ...
                const filteredData = ensuredData.filter((dataPoint) => dataPoint.x >= sevenDaysAgo);
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen  text-white p-4">
            {/* Header */}
            <h2 className="text-4xl mb-8">Generating yield</h2>
